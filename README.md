...
1. Установите [Thonny](https://thonny.org/), в настройках выберите
   интерпретатор *MicroPython (Raspberry Pi Pico)* и порт платы
2. Откройте каждый `.py` из этой папки и сохраните на устройство
   (*Файл → Сохранить как → MicroPython device*), имя файла не менять
3. Нажмите кнопку reset на плате (или переткните USB)
